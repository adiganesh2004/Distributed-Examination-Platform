...
useEffect(() => {
    ...
    setQuestions([
        ...
        {
          id: 3,
          question: "Which language is React written in?",
      ...
      <Card className="p-6 space-y-4 shadow-lg">
        ...
        <div className="space-y-2">
          ...
          {selectedQuestions.map((qId, i) => (
            <div key={i} className="flex flex-col md:flex-row md:items-center space-y-2 md:space-y-0 md:space-x-2">
              {selectedQuestions[i] === null ? (
                ...
                <div className="font-semibold">{questions.find(q => q.id == selectedQuestions[i])?.question}</div>
                    <div className="space-y-1">
                        {questions
                        .find(q => q.id == selectedQuestions[i])
                        ?.options.map((opt, idx) => {
                            const correctIndex = questions.find(q => q.id == selectedQuestions[i]).answerIndex;
                            return (
                            <div
                                key={idx}
